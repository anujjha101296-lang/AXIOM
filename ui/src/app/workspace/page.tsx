...
interface GraphNodeMetadata {
  ...
  lean_file?: string;
  proof_path?: Array<[string, string]>;
}
...
export default function Home() {
  ...
  const fetchGraph = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        const currentNodes = data.nodes.map((node, idx) => {
  ...
  const handleIngest = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = (await res.json()) as { claims_extracted: number; concepts_extracted: number };
      ...
      setIngestResult(`Error: ${err instanceof Error ? err.message : 'Unable to reach the API.'}`);
    }
            ...
            {selectedNode.metadata?.proof_path && (
              <div className="bg-slate-950 p-3 rounded border border-slate-800 space-y-2">
                ...
                <ol className="text-xs font-mono space-y-1 list-decimal list-inside text-blue-300">
                  {selectedNode.metadata.proof_path.map((step, idx) => (
