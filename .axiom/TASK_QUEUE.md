...
| 4 | S1-SCI-006 | Persist and stream research lifecycle events incrementally. | S1-SCI-004 | Each state transition is recorded and the workspace can observe run progress without waiting for completion. | **Verified: lifecycle, replay, async, and SSE tests pass in CI** |
| 5 | S1-SCI-005 | Add provider-neutral agent adapters: OpenAI Agents SDK + Ollama. | S1-SCI-002 | Interchangeable planners produce the same structured scientific plan contract; deterministic execution remains authoritative. | **Active: adapters + safety contracts implemented; CI pending** |
