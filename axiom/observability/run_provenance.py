"""
H1-OBS — Unified run provenance records for SCEP (EPIC-002) and RVP.

Every scientific evaluation run records inputs, runtime, configuration, environment,
and evidence tier in a queryable `run_provenance` table. Reuses existing SCEP and
RVP result tables; provenance is additive and does not duplicate score payloads.
"""
...
RunType = Literal["scep", "rvp", "sme", "aca", "kernel"]

_EVIDENCE_RANK = {
    EvidenceState.UNAVAILABLE.value: 0,
    EvidenceState.BASELINE.value: 1,
    EvidenceState.ESTIMATED.value: 2,
    EvidenceState.SIMULATED.value: 3,
    EvidenceState.MEASURED.value: 4,
}
...
def rollup_evidence_tier(dimension_states: dict[str, str]) -> dict[str, Any]:
    ...
    has_simulated = any(s == EvidenceState.SIMULATED.value for s in dimension_states.values())
    has_baseline = any(
        s in (EvidenceState.BASELINE.value, EvidenceState.ESTIMATED.value, EvidenceState.UNAVAILABLE.value)
        for s in dimension_states.values()
    )
    weakest = min(dimension_states.values(), key=lambda s: _EVIDENCE_RANK.get(s, 0))
    strongest_measured = all(
        s in (EvidenceState.MEASURED.value, EvidenceState.SIMULATED.value)
        for s in dimension_states.values()
    )

    if strongest_measured and not has_simulated:
        aggregate = EvidenceState.MEASURED.value
    elif has_simulated:
        aggregate = EvidenceState.SIMULATED.value
    else:
        aggregate = weakest
...
def build_scep_provenance(
    *,
    ...
    dimension_states = {
        s.dimension.value: s.evidence_state.value for s in snapshot.dimension_scores
    }
    return RunProvenance(
        ...
        evidence_tier=rollup_evidence_tier(dimension_states),
        runtime={
...
def build_rvp_provenance(
    *,
    ...
    evidence_state = (
        EvidenceState.MEASURED.value
        if verification_invoked
        else EvidenceState.MEASURED.value
    )
    return RunProvenance(
        run_id=run_id,
        run_type="rvp",
